...
import {
  ...
  CircleAlert, ClipboardList, FileSearch, FileText, Pencil, Plus,
  ReceiptText, RefreshCw, Search, X,
} from "lucide-react";
...
export default function CasesDashboard({ onOpenWizard }: { onOpenWizard: (item: CaseItem) => void }) {
  ...
  const openStatusEditor = (item: CaseItem) => setStatusEditor({
    ...
    taxBureauCode: item.taxBureauCode || "", nextFollowUpDate: item.nextFollowUpDate || "",
  });
  ...
  const openTax = (item: CaseItem) => setTaxDialog({
    caseId: item.id, bureauCode: item.taxBureauCode || "", sessionId: "", captchaUrl: "",
    captchaText: "", results: [], message: "請選擇主管國稅局，再載入官方驗證碼。", busy: false,
  });
  const loadTaxCaptcha = async () => {
    if (!taxDialog?.bureauCode) return;
    setTaxDialog({ ...taxDialog, busy: true, message: "正在取得驗證碼…", results: [] });
    try {
      const result = await api<{ sessionId: string; captchaUrl: string }>(`/api/cases/${taxDialog.caseId}/tax-query-session`, { method: "POST", body: JSON.stringify({ bureauCode: taxDialog.bureauCode }) });
      setTaxDialog((current) => current ? { ...current, sessionId: result.sessionId, captchaUrl: `${result.captchaUrl}?v=${Date.now()}`, captchaText: "", message: "請輸入圖片中的驗證碼。", busy: false } : null);
  ...
  const applyTax = async (index: number) => {
    ...
    try {
      await api(`/api/cases/${taxDialog.caseId}/apply-tax-progress`, { method: "POST", body: JSON.stringify({ sessionId: taxDialog.sessionId, candidateIndex: index }) });
      const id = taxDialog.caseId; setTaxDialog(null); setNotice("已帶入國稅局案件進度"); await load(true); await refreshDetail(id);
