...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = await request.json() as { employeeId?: number; stage?: keyof typeof stageProgress; action?: "complete" | "restore" | "keep_active" }; const now = new Date().toISOString(); const db = await getRawDb(); const caseId = Number(id);
    ...
    else if (body.action === "keep_active") await db.prepare("UPDATE cases SET updated_at = ? WHERE id = ? AND status = 'ongoing'").bind(now, caseId).run();
