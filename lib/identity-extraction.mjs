...
export function selectIdentityResult(orderedFileIds, results) {
  ...
  const complete = completed.find(
    (item) => item.name && isValidTaiwanNationalId(item.nationalId),
  );
  if (complete) return { state: "success", ...complete };
  const partial = completed.find((item) => item.name || item.nationalId);
  return partial
