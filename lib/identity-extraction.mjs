...
export function selectIdentityResult(orderedFileIds, results) {
  ...
  const postalResult = completed.find((item) => item.contactPostalCode);
  if (nameResult && idResult)
    return {
      ...
      contactPostalCode: postalResult?.contactPostalCode ?? "",
  ...
  return partial
    ? {
        ...
        contactPostalCode:
          postalResult?.contactPostalCode ?? partial.contactPostalCode ?? "",
      }
    : { state: "review" };
}
